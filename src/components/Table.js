...
const RowNew = ({club, index, sep, isThird}) => {
  const getSign = (value) => {
    return value > 0 ? '+' : value < 0 ? '–' : '±'
  }
  return (
...
export const TableNew = ({ table, separators, isThird = false }) => {
  return (
    <div sx={{ width: "100%", height: "100vh", display: "flex", flexDirection: "column", justifyContent: "center" }}>
      {table.map((club, index) => {
        if (separators.includes(index)) return <RowNew club={club} key={club.team} index={index} sep={true} isThird={isThird} />
        return <RowNew club={club} key={club.team} index={index} isThird={isThird} />
      })
      }
  </div>
  )
