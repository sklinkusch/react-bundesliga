...
function Liga2_23_24({ title }: Props) {
  ...
  useEffect(() => {
    const url = "https://buli-api.vercel.app/liga2men?season=2023-24"
    // const url = "http://localhost:3500/liga2men?season=2023-24"
    fetch(url)
        ...
        if (typeof data === "object" && Object.keys(data).length > 0) {
          ...
          if (apiMatches && !queryParams.get("day")) {
            ...
            if (sortedKeys.length > 0) {
              navigate(`/liga2/23-24?day=${Number(sortedKeys[0])}`)
            }
          }
        }
      })
